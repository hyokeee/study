...
public class MapTest {
	...
	/*
		Map ==> key값과 value값을 한 쌍으로 관리하는 객체
			- key값은 중복을 허용하지 않고 순서가 없다.(Set의 특징과 같다.)
			- value값은 중복을 허용한다.	
	 */	
		...
		//Set 특성 갖기 때문에 집어넣은 순서와 출력 순서가 다르다.
...
//		System.out.println("삭제된 자료의 value값 => " + removeTEl);
		
		// 'key값'이 존재하는지 여부를 나타내는 메서드 : containsKey(key값)
		...
		// keySet() 메서드 ==> Map의 모든 'key값'들을 Set형으로 반환한다.
		...
		// 방법2 : 'value값'만 읽어와서 처리하기
		//(하지만 key값을 같이 이용하는 방식이 더 좋다.)
		//		==> values() 메서드를 이용한다.
		...
		// 'key값'은 번호(int)로 하고 'value값'은 Member의 인스턴스로 하는 Map객체 생성하기
		HashMap<Integer,Member> memberMap = new HashMap<>();
		
		memberMap.put(1, new Member(1, "홍길동", "010-1111-1111"));
		
		Member mem = new Member(2,"이순신","010-2222-2222");
		memberMap.put(2,mem);
		
		System.out.println("map의 개수 : " + memberMap.size());
		System.out.println("memberMap => " + memberMap);
		System.out.println("-------------------------------------------");
		System.out.println(memberMap.get(1).getName());
		System.out.println(memberMap.get(1).getNum());
