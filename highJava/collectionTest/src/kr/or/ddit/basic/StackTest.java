...
//웹브라우저의 앞으로 가기, 뒤로 가기 기능 구현(Stack이용)
class Browser{
	...
	public void goURL(String url) {
		...
		if(currentURL != null && !"".equals(currentURL)) { //현재 페이지가 있으면
			back.push(currentURL);							//현재 페이지를 이전페이지에 저장
		}
		currentURL = url;  //현재 페이지를 변경(=방문 완료)
		forward.clear();
	}
	
	//뒤로 가기
	public void goBack() {
		//isEmpty()메서드 ==> 컬렉션이 비어있으면 true, 그렇지 않으면 false를 반환한다.
		if(!back.isEmpty()) {	//back스택이 비어있지 않으면...
			forward.push(currentURL); //현재 페이지를 forward 스택에 추가한다.
			currentURL = back.pop();	//back 스택에서 1개의 요소를 꺼내와 현재페이지로 설정한다.
		}
	}
