...
public class JdbcBoardController {
	...
	public void boardStart() {
		...
		int choice = -1;
		while (true) {
			if(choice!=3) {		// 선택한 메뉴가 '검색'이 아니면 그 전에 입력했던 검색단어를 없앤다.
				searchTitle = null;
			}
	...
	// 게시판 목록을 보여주고 메뉴를 나타내며 사용자가 입력한 메뉴 번호를 입력받아 반환하는 메서드
	private int displayMenu(String title) {
		...
		System.out.println("----------------------------------------");
		if(title == null) {
			List<JdbcBoardVO> boardList = service.getAllBoard();
			...
			List<JdbcBoardVO> boardList = service.getSearchBoard(title);
			for (JdbcBoardVO boardVo : boardList) {
				System.out.println(boardVo.getBoard_no() + "\t" + boardVo.getBoard_title() + "\t"
						+ boardVo.getBoard_writer() + "\t" + boardVo.getBoard_cnt());	
			}
