...
/*
	UDP방식 : 비연결 지향, 신뢰성 없음, 데이터가 순서대로 도착한다는 보장이 없다.
	
	DatagranSocket 객체와  DatagramPacket객체를 이용해서 통신한다.
	-DatagramSocket : 데이터의 송수신과 관련된 작업을 수행한다. (우체부)
	-DatagramPacket : 주고 받는 데이터와 관련된 작업을 수행한다. (소포,편지)
		==> 수신용 생성자와 송신용 생성자를 따로 제공한다.
		
	TCP방식의 경우에는 스트림객체를 이용해서 송수신하지만
	...
	public static void main(String[] args) {
		try {
			...
			while(true) {
				...
				// 상대방이 보낸 메시지 출력하기
				// 상대방이 보낸 데이터는 '수신용패킷객체(inpacket)'의 getData()메서드로 구할 수 있고,
				// 					  수신용 패킷 객체를 생성할 때 지정한 byte형 배열(bMsg)에도 저장된다.
				// 상대방이 보낸 데이터의 크기는 '수신용 패킷 객체'의 getLength() 메서드로 구할 수 있다.
				
				// 수신받은 데이터는 byte형 배열이기 때문에 이것을 문자열로 변환해야 한다.
				
				// 방법1 ==> 패킷 객체 생성할 때 사용한 byte형 배열(bMsg) 이용하기
