...
public class ResponseTest02 extends HttpServlet {
	 ...
	 */
	/*
	 * redirect는 Request객체를 공유하지 못한다.(브라우저에서 새롭게 요청하기 때문에)
	 * request.setAttribute("tel", "010-1234-5678");
	 * 
	 * // 이동하기
	 * 
	 * // redirect로 이동은 response객체의 sendRedirect() 메서드를 사용한다. // 이 때 이동할 URL주소는 전체
	 * URL경로를 지정해 주어야 한다. response.sendRedirect(request.getContextPath() +
	 * "/redirectTest.do");
	 */
		...
		// URL 경로에 한글이 포함될 경우에는 URLEncoder객체를 이용하여 인코딩해서 지정해주어야한다.
