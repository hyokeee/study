...
public class CookieAdd extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// 1. Cookie객체를 생성한다. ==> '쿠키이름'과 '쿠키값'을 문자열로 지정한다.
		// 형식) Cookie cookie변수 = new Cookie("쿠키이름","쿠키값");
		// ==> '쿠키값'으로 한글을 사용할 경우에는 URLEncoder.encode()메서드를 인코딩한 후 저장한다.
		...
		//Cookie ageCookie = new Cookie("age",26+"");
		//Cookie ageCookie = new Cookie("age",String.valueOf(26)); 숫자를 문자로 변환해서 값으로 줘야한다.
		Cookie genderCookie = new Cookie("gender","Male");
		
		// 2. Cookie 속성 설정
		/*
		  쿠키변수.setPath("적용경로");     ==> 지정한 경로와 그 허위 경로에서 사용 가능하다.
		  								==> 생략하면 쿠키를 저장할 당시의 경로가 자동으로 설정된다.
		  								
		  쿠키변수.setMaxAge(유지시간);		==> 단위(초), (-1 : 웹 브라우저가 종료될때까지 유지한다.(기본값), 0 : 즉시 삭제된다.)
