...
public class ResponseTest01 extends HttpServlet {
	 ...
	 */
		...
		//forward 방식으로 다른 문서로 이동하기
		//Resquest객체의  getRequestDispatcher()메서드에 이동할 서블릿이나 JSP를 지정해 주는데
		// 현재 URI경로 중에서 ContextPath이후의 경로를 지정해 준다
		
		// 이동할 문서의 전체 URL경로 예) http.//localhost/webTest.fosardTest.do
		// 위의 경로중 ContextPath(지금은 /webTest) 이후의 경로로 지정한다.
		RequestDispatcher rd = request.getRequestDispatcher("/forwardTest.do");
		
		// forwarding 작업 수행(이때 request객체와 response객체를 인수값으로 넣어준다.)
