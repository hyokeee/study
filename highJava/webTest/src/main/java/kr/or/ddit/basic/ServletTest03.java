...
2. 컨테이너는 web.xml에 정의된 url패턴을 확인하여 어느 서블릿을 통해 처리해야 할지를 검색한다.
 	(이 때, 서블릿이 로딩이 안된 경우에는 로딩 작업을 한다. 처음 로딩시에는 init()메서드를 호출한다.)
 	(Servlet 버전 3.0 이상에서는  애노테이션(@WebServlet)으로 설정할 수 있다.)
3. Servlet Container는 개별 요청을 처리할 쓰레드를 생성하여 해당 서블릿 객체의
   service()메서드를 호출한다.
   (이 때, HttpServletRequest객체와 HttpServletResponse객체를 생성하여 파라미터로
   넘겨 준다.)
4. service()메서드는 전송방식(GET, POST등 메서드 타입)을 체크하여 적절한 메서드를 호출한다.
   (doGet(), doPost(), doPut(), doDelete() 등...)
5. 요청 및 응답 처리가 모두 완료되면 HttpServletRequest객체와 HttpServletRespoonse객체는
   자동으로 소멸한다.
