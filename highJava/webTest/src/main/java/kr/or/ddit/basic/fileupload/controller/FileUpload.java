...
/*
  - Servlet 3.0 이상에서는 파일 업로드를 하려면 @MultipartConfig 애노테이션을 설정해야 한다.
  ...
  2) fileSizeThreshold ==> 이 곳에 지정한 값보다 큰 파일이 전송되면 location에 지정한 임시 디렉토리에
  				파일을 저장해놓고 처리한다.(단위 : byte, 기본값 : 0 (무조건 임시 디렉토리를 사용한다.))
...
public class FileUpload extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		for(Part part : request.getParts()) {
			...
			if(!"".equals(fileName)) {	//파일인지 검사
				...
				try {
					//upload된 파일 저장하기 ==> Part 객체의 write() 메서드를 이용한다.
	...
	1) 파일이 아닌 일반 데이터인 경우
	-------------------iuhukhjhasdf ==> 각각의 Part를 구분하는 선
	content-disposition:form-data; name="username" ==> 파라미터명
										=>빈 줄
	hong   								==>파라미터값
	
	2) 파일일 경우
	-----------------asdfasdf ==> 각각의 Part를 구분하는 선
	content-disposition: form-data; name="upfile11"; filename = "test1.txt" ==> 파일 정보
