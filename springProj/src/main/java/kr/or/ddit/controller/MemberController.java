...
public class MemberController {
	...
	//폼 객체의 속성명과 스프링 폼 태그의 modelAttribute 속성값이 일치해야 함
	@GetMapping("/registerForm02")
	public String registerForm02(Model model) {
		//모델의 속성명에 memberVO를 지정하고 폼 객체를 모델에 추가함
		//model.addAttribute("gaeddongi",new MemberVO()); //(x)
		model.addAttribute("memberVO",new MemberVO());	  //(o)
		//forwarding
		return "member/registerForm02";
	}
	
	//컨트롤러 메서드의 매개변수로 자바빈즈 객체가 전달이 되면
	// forwarding 시 뷰(registerForm05.jsp)로 memberVO를 전달함
	//컨트롤러는 자바빈즈 규칙에 맞는 객체가 있다면 뷰로 전달해준다
	@GetMapping("/registerForm05")
	public String registerForm05(MemberVO memberVO) {
		//폼 객체의 속성명은 직접 지정하지 않으면 : 골뱅이 ModelAttribute("속성명")을 생략
	...
	//골뱅이 ModelAttribute 애너테이션으로 폼 객체의 속성명을 직접 지정할 수 있음
	...
	public String registerForm07(@ModelAttribute("addressVO") AddressVO addressVO) {
		addressVO.setZonecode("11111");
		addressVO.setAddress("대전 서구 탄방동");
		addressVO.setBuildingName("402호");
	...
	public String registerForm08(@ModelAttribute("memberVO") MemberVO memberVO,
		...
		memberVO.setUserId("gaeddongi");
		memberVO.setUserName("개똥이");
		// 값을 setting하여 뷰에 전달해도 패스워드 필드에 반영되지 않음.
...
//		memberVO.setGender("Male");
		...
		nationalityMap.put("Austraila","오스트레일리아");
		
		model.addAttribute("nationalityMap",nationalityMap);
		memberVO.setNationality("Korea");
	...
	//				,gender=Male,nationality=Korea,cars=[genesis,sonata]}
	...
	public String registerForm08Post(@Validated MemberVO memberVO, BindingResult result
			,Model model) {
		log.info("memberVO : " + memberVO);
		log.info("result.hasErrors() : " + result.hasErrors());
		//유효성 검증 실패
		if(result.hasErrors()) {//true(오류발생)
