...
<form:form modelAttribute="memberVO" method="post" action="/member/registerForm08Post">
	...
	<p>
		...
		<font color="red"><form:errors path="addressVO.zonecode"/></font>
		주소 : <form:input path="addressVO.address"/>
		<font color="red"><form:errors path="addressVO.address"/></font>
		상세주소 : <form:input path="addressVO.buildingName"/>
		<font color="red"><form:errors path="addressVO.buildingName"/></font>
	</p>
	<p>
		<form:label path="cardVOList[0].no">카드1 - 번호 : </form:label>
		<form:input path="cardVOList[0].no"/> <br/>
		<font color="red"><form:errors path="cardVOList[0].no"/></font>
		<form:label path="cardVOList[0].validMonth">카드1 - 유효연월 : </form:label>
		<form:input path="cardVOList[0].validMonth"/> <br/>
		<font color="red"><form:errors path="cardVOList[0].validMonth"/></font>
		<hr/>
		<form:label path="cardVOList[1].no">카드2 - 번호 : </form:label>
		<form:input path="cardVOList[1].no"/> <br/>
		<font color="red"><form:errors path="cardVOList[1].no"/></font>
		<form:label path="cardVOList[1].validMonth">카드2 - 유효연월 : </form:label>
		<form:input path="cardVOList[1].validMonth"/> <br/>
	...
	<p>
		<form:label path="nationality">국적 : </form:label>
		<!-- select 박스 요소 select -->
		<form:select path="nationality" items="${nationalityMap }"/>
	</p>
	<p>
		<form:label path="cars">보유자동차 : </form:label>
		<!-- select 다중선택(multiple) --> 
